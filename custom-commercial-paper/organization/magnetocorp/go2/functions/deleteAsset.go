...
func DeleteAsset(idUser string, cid string) ([]byte, error) {
	...
	defer log.Printf("==============> Delete of cid %v successfully complete", cid)
	defer gw.Close()
	...
	result, err := contract.SubmitTransaction("DeleteAsset", cid)
	if err != nil {
		log.Fatalf("Failed to evaluate transaction: %v", err)
	}
	log.Println("il risultato della delete" + string(result))
	return result, err

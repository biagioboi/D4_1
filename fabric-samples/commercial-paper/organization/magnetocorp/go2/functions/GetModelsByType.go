...
func GetModelsByType(idUser string, ModelType string) ([]byte, error) {
	...
	defer log.Println("==============> GetModels program complete")
	defer gw.Close()
	...
	log.Println("--> Evaluate Transaction: GetModels, returns all the current assets on the ledger:")
	result, err := contract.EvaluateTransaction("GetModels", ModelType)
	if err != nil {

...
func Buy(function string, id string, issuer string, paperNumber string, currentOwner string, newOwner string, price string, purchaseDateTime string) ([]byte, error) {

	log.Println("==============> SUBMIT BUY ")
	...
	defer log.Println("==============> Buy program complete")
	defer gw.Close()
	...
	result, err := contract.SubmitTransaction(function, issuer, paperNumber, currentOwner, newOwner, price, purchaseDateTime)
	if err != nil {
		return result, fmt.Errorf("failed to Submit transaction: %v", err)
	}
	fmt.Println(string(result))
	log.Printf("==============> Process buy transaction response. %v", string(result))
